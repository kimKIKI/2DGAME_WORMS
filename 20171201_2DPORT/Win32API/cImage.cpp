...
// 빈 비트맵 셋업
void cImage::SetupEmpty(int width, int height)
{
	// DC 가져오기
...
HBITMAP cImage::GetHBITMAPFromImageFile(const char* fileName)
{
	...
	Bitmap* bitmap = Bitmap::FromFile(wFileName, false);
	if (bitmap)
	{
...
// 이미지 파일 셋업
void cImage::SetupImage(const char* fileName, int width, int height, bool isTrans, COLORREF transColor)
{
	// DC 가져오기
	HDC hdc = GetDC(g_hWnd);
	...
	m_pImageInfo->dwResID = 0;
	m_pImageInfo->hMemDC = CreateCompatibleDC(hdc);
	m_pImageInfo->hBit = (HBITMAP)GetHBITMAPFromImageFile(fileName);
	m_pImageInfo->hOldBit = (HBITMAP)SelectObject(m_pImageInfo->hMemDC, m_pImageInfo->hBit);
	m_pImageInfo->nWidth = width;
	m_pImageInfo->nHeight = height;
	m_pImageInfo->nFrameWidth = width;
...
// 프레임 이미지 셋업
void cImage::SetupFrameImage(const char * fileName, int width, int height, int frameX, int frameY, bool isTrans, COLORREF transColor)
{
	// DC 가져오기
	HDC hdc = GetDC(g_hWnd);
...
void cImage::SetupFrameImage(const char * fileName, int width, int height, int frameX, int frameY,
...
void cImage::FrameRender(HDC hdc, int destX, int destY, int sourX, int sourY, int sizeX, int sizeY)
{
	// GdiTransparentBlt : 비트맵을 불러올때 특정색상을 제외하고 복사를 하는 함수
	GdiTransparentBlt(
		hdc,						// 복사 할 장소의 DC
...
//알파 프레임 렌더
void cImage::AlphaFrameRender(HDC hdc, int destX, int destY, BYTE alpha)
{
	// 알파블렌드 처음 사용시 초기화
	if (!m_pBlendImage) SetupForAlphaBlend();
	...
	{
		//1. 출력해야 될 DC에 그려져 있는 내용을 블렌드이미지에 그려준다
		BitBlt(m_pBlendImage->hMemDC, 0, 0, m_pImageInfo->nFrameWidth, m_pImageInfo->nFrameHeight,
			hdc, destX, destY, SRCCOPY);
		//2. 출력해야 될 이미지를 블렌드에 그려준다(마젠타 값을 없애준다)
		GdiTransparentBlt(
		...
		//3. 블렌드 DC를 출력해야 할 DC에 그린다
		GdiAlphaBlend(hdc, destX, destY, m_pImageInfo->nFrameWidth, m_pImageInfo->nFrameHeight,
			m_pBlendImage->hMemDC, 0, 0, m_pImageInfo->nFrameWidth, m_pImageInfo->nFrameHeight, m_stBlendFunc);
	}
	else
	{
		// 알파블렌드 옵션 값을 사용해서 그린다.
